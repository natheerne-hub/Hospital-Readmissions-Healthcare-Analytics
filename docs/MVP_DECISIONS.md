...
### Consequence
The MVP may show hospital performance signals, condition comparisons, reporting completeness, data-quality indicators, and persistent ERR patterns. A separate patient-level research candidate has now been trained on the UCI Diabetes dataset, but clinical probability remains locked until external validation, calibration review, workflow evaluation, and governance approval.
...
### Decision
Only report patient-model metrics generated by the versioned, reproducible evaluation evidence. Never present research performance as clinical validity.
